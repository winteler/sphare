...
use sphare_core_user::role::ssr::{get_sphere_role_vec, get_user_sphere_role, get_user_sphere_role_by_sphere_id, set_user_admin_role, set_user_sphere_role};
...
#[tokio::test]
async fn test_get_user_sphere_role() {
    ...
    let sphere_2 = sphere::ssr::create_sphere("2", "sphere", false, &user_a, &db_pool).await.expect("Should create sphere");
    let sphere_3 = sphere::ssr::create_sphere("3", "sphere", false, &user_b, &db_pool).await.expect("Should create sphere");
    ...
    let user_a_sphere_1_role = get_user_sphere_role(user_a.person_id, &sphere_1.sphere_name, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_a_sphere_1_role.person_id, user_a.person_id);
    ...
    assert_eq!(user_a_sphere_1_role.grantor_id, user_a.person_id);
    assert_eq!(user_a_sphere_1_role.permission_level, PermissionLevel::Lead);

    let user_b_sphere_1_role = get_user_sphere_role(user_b.person_id, &sphere_1.sphere_name, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_b_sphere_1_role.person_id, user_b.person_id);
    ...
    assert_eq!(user_b_sphere_1_role.grantor_id, user_a.person_id);
    assert_eq!(user_b_sphere_1_role.permission_level, PermissionLevel::Manage);

    let user_c_sphere_1_role = get_user_sphere_role(user_c.person_id, &sphere_1.sphere_name, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_c_sphere_1_role.person_id, user_c.person_id);
    ...
    assert_eq!(user_c_sphere_1_role.grantor_id, user_a.person_id);
    assert_eq!(user_c_sphere_1_role.permission_level, PermissionLevel::Moderate);

    let user_a_sphere_2_role = get_user_sphere_role(user_a.person_id, &sphere_2.sphere_name, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_a_sphere_2_role.person_id, user_a.person_id);
    ...
    assert_eq!(user_a_sphere_2_role.grantor_id, user_a.person_id);
    assert_eq!(user_a_sphere_2_role.permission_level, PermissionLevel::Lead);

    let user_b_sphere_2_role = get_user_sphere_role(user_b.person_id, &sphere_2.sphere_name, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_b_sphere_2_role.person_id, user_b.person_id);
    ...
    assert_eq!(user_b_sphere_2_role.grantor_id, user_a.person_id);
    assert_eq!(user_b_sphere_2_role.permission_level, PermissionLevel::Ban);

    let user_c_sphere_2_role = get_user_sphere_role(user_c.person_id, &sphere_2.sphere_name, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_c_sphere_2_role.person_id, user_c.person_id);
    ...
    assert_eq!(user_c_sphere_2_role.grantor_id, user_a.person_id);
    assert_eq!(user_c_sphere_2_role.permission_level, PermissionLevel::Moderate);

    let user_a_sphere_3_role = get_user_sphere_role(user_a.person_id, &sphere_3.sphere_name, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_a_sphere_3_role.person_id, user_a.person_id);
    ...
    assert_eq!(user_a_sphere_3_role.grantor_id, user_b.person_id);
    assert_eq!(user_a_sphere_3_role.permission_level, PermissionLevel::None);

    let user_b_sphere_3_role = get_user_sphere_role(user_b.person_id, &sphere_3.sphere_name, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_b_sphere_3_role.person_id, user_b.person_id);
    ...
    assert_eq!(user_b_sphere_3_role.grantor_id, user_b.person_id);
    assert_eq!(user_b_sphere_3_role.permission_level, PermissionLevel::Lead);

    assert_eq!(get_user_sphere_role(user_c.person_id, &sphere_3.sphere_name, &db_pool).await, Err(AppError::NotFound));
}

#[tokio::test]
async fn test_get_user_sphere_role_by_sphere_id() {
    let db_pool = get_db_pool().await;
    let user_a = create_user("a", &db_pool).await;
    let user_b = create_user("b", &db_pool).await;
    let user_c = create_user("c", &db_pool).await;
    ...
    let user_a_sphere_1_role = get_user_sphere_role_by_sphere_id(user_a.person_id, sphere_1.sphere_id, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_a_sphere_1_role.person_id, user_a.person_id);
    assert_eq!(user_a_sphere_1_role.sphere_id, sphere_1.sphere_id);
    assert_eq!(user_a_sphere_1_role.sphere_name, sphere_1.sphere_name);
    assert_eq!(user_a_sphere_1_role.grantor_id, user_a.person_id);
    ...
    let user_b_sphere_1_role = get_user_sphere_role_by_sphere_id(user_b.person_id, sphere_1.sphere_id, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_b_sphere_1_role.person_id, user_b.person_id);
    assert_eq!(user_b_sphere_1_role.sphere_id, sphere_1.sphere_id);
    assert_eq!(user_b_sphere_1_role.sphere_name, sphere_1.sphere_name);
    assert_eq!(user_b_sphere_1_role.grantor_id, user_a.person_id);
    ...
    let user_c_sphere_1_role = get_user_sphere_role_by_sphere_id(user_c.person_id, sphere_1.sphere_id, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_c_sphere_1_role.person_id, user_c.person_id);
    assert_eq!(user_c_sphere_1_role.sphere_id, sphere_1.sphere_id);
    assert_eq!(user_c_sphere_1_role.sphere_name, sphere_1.sphere_name);
    assert_eq!(user_c_sphere_1_role.grantor_id, user_a.person_id);
    ...
    let user_a_sphere_2_role = get_user_sphere_role_by_sphere_id(user_a.person_id, sphere_2.sphere_id, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_a_sphere_2_role.person_id, user_a.person_id);
    assert_eq!(user_a_sphere_2_role.sphere_id, sphere_2.sphere_id);
    assert_eq!(user_a_sphere_2_role.sphere_name, sphere_2.sphere_name);
    assert_eq!(user_a_sphere_2_role.grantor_id, user_a.person_id);
    ...
    let user_b_sphere_2_role = get_user_sphere_role_by_sphere_id(user_b.person_id, sphere_2.sphere_id, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_b_sphere_2_role.person_id, user_b.person_id);
    assert_eq!(user_b_sphere_2_role.sphere_id, sphere_2.sphere_id);
    assert_eq!(user_b_sphere_2_role.sphere_name, sphere_2.sphere_name);
    assert_eq!(user_b_sphere_2_role.grantor_id, user_a.person_id);
    ...
    let user_c_sphere_2_role = get_user_sphere_role_by_sphere_id(user_c.person_id, sphere_2.sphere_id, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_c_sphere_2_role.person_id, user_c.person_id);
    assert_eq!(user_c_sphere_2_role.sphere_id, sphere_2.sphere_id);
    assert_eq!(user_c_sphere_2_role.sphere_name, sphere_2.sphere_name);
    assert_eq!(user_c_sphere_2_role.grantor_id, user_a.person_id);
    ...
    let user_a_sphere_3_role = get_user_sphere_role_by_sphere_id(user_a.person_id, sphere_3.sphere_id, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_a_sphere_3_role.person_id, user_a.person_id);
    assert_eq!(user_a_sphere_3_role.sphere_id, sphere_3.sphere_id);
    assert_eq!(user_a_sphere_3_role.sphere_name, sphere_3.sphere_name);
    assert_eq!(user_a_sphere_3_role.grantor_id, user_b.person_id);
    ...
    let user_b_sphere_3_role = get_user_sphere_role_by_sphere_id(user_b.person_id, sphere_3.sphere_id, &db_pool).await.expect("get_user_sphere_role should return user role.");
    assert_eq!(user_b_sphere_3_role.person_id, user_b.person_id);
    assert_eq!(user_b_sphere_3_role.sphere_id, sphere_3.sphere_id);
    assert_eq!(user_b_sphere_3_role.sphere_name, sphere_3.sphere_name);
    assert_eq!(user_b_sphere_3_role.grantor_id, user_b.person_id);
    ...
    assert_eq!(get_user_sphere_role_by_sphere_id(user_c.person_id, sphere_3.sphere_id, &db_pool).await, Err(AppError::NotFound));
}
...
async fn test_get_sphere_role_vec() -> Result<(), AppError> {
    ...
    let user_a_sphere_role = get_user_sphere_role(user_a.person_id, &sphere.sphere_name, &db_pool).await.expect("User a should have lead role.");
...
async fn test_set_user_sphere_role() {
    ...
    let lead_user = User::get(lead_user.person_id, &db_pool)
        .await
        .expect("Should be able to reload lead_user.");

    let lead_role = get_user_sphere_role(lead_user.person_id, &sphere.sphere_name, &db_pool).await.expect("Should get lead role");
    ...
    assert_eq!(moderate_role.person_id, moderator.person_id);
    assert_eq!(moderate_role.sphere_id, sphere.sphere_id);
    assert_eq!(moderate_role.sphere_name, sphere.sphere_name);
    assert_eq!(moderate_role.grantor_id, lead_user.person_id);
    ...
    let moderator = User::get(moderator.person_id, &db_pool)
        .await
    ...
    assert_eq!(manage_role.person_id, moderator.person_id);
    assert_eq!(manage_role.sphere_id, sphere.sphere_id);
    assert_eq!(manage_role.sphere_name, sphere.sphere_name);
    assert_eq!(manage_role.grantor_id, lead_user.person_id);
    ...
    assert_eq!(deleted_moderator_role.person_id, moderate_role.person_id);
    ...
    let prev_lead_user = User::get(lead_user.person_id, &db_pool)
        .await
    ...
    assert_eq!(deleted_mod_role_2.person_id, moderate_role_2.person_id);
    ...
    assert_eq!(deleted_lead_role.person_id, lead_role.person_id);
...
async fn test_set_user_admin_role() -> Result<(), AppError> {
    ...
    assert_eq!(sql_admin.person_id, admin.person_id);
    assert_eq!(sql_admin.admin_role, AdminRole::Admin);
    let admin = User::get(admin.person_id, &db_pool).await.expect("Should be able to reload admin.");
    ...
    assert_eq!(sql_moderator.person_id, moderator.person_id);
    assert_eq!(sql_moderator.admin_role, AdminRole::Moderator);
    let moderator = User::get(moderator.person_id, &db_pool).await.expect("Should be able to reload moderator.");
