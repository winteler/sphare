...
use crate::apub_factory::{get_apub_community_tag, get_mock_server_url, mock_apub_group, mock_apub_group_moderators, mock_apub_persons};
use crate::apub_utils::{test_sphere_category_vec, test_sphere_role_vec};
...
async fn test_apub_sphere_object_from_json() {
    ...
    test_sphere_category_vec(&sphere, &group.tags, &db_pool).await;
    ...
    group.tags.push(get_apub_community_tag("funny", "For funny content", &get_mock_server_url(&mock_server)));
