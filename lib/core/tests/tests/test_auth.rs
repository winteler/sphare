...
use sphare_core_user::user::ssr::{create_or_update_user, DbUser};
...
#[tokio::test]
async fn test_db_user_get_by_username() -> Result<(), AppError> {
...
async fn test_user_get() -> Result<(), AppError> {
    ...
    assert_eq!(
        ban_user_from_sphere(test_user.person_id, sphere_c.sphere_id, post_c.post_id, None, rule.rule_id, Some(0), &creator_user, &db_pool).await.expect("User ban should be created for sphere c."),
        None
    );
    let sphere_ban_d = ban_user_from_sphere(test_user.person_id, sphere_d.sphere_id, post_d.post_id, None, rule.rule_id, Some(1), &creator_user, &db_pool)
        .await?
        .expect("User should have ban for sphere d.");
    ban_user_from_sphere(test_user.person_id, sphere_e.sphere_id, post_e.post_id, None, rule.rule_id, None, &creator_user, &db_pool).await
        .expect("User ban should be created for sphere e.")
        .expect("User should have ban for sphere e.");

    let result_user = User::get(test_user.person_id, &db_pool).await.expect("result_user should be available in DB.");

    assert_eq!(result_user.user_id, test_user.user_id);
    assert_eq!(result_user.person_id, test_user.person_id);
...
async fn test_user_check_can_set_user_sphere_role() -> Result<(), AppError> {
    ...
    set_user_sphere_role(&simple_mod.username, &sphere.sphere_name, PermissionLevel::Ban, &lead_user, &db_pool)
        .await.expect("Moderate role should be assignable by lead_user.");
    let manage_mod = User::get(manage_mod.person_id, &db_pool).await.expect("Should be able to get elect mod.");
    let simple_mod = User::get(simple_mod.person_id, &db_pool).await.expect("Should be able to get simple mod.");
    admin.admin_role = AdminRole::Admin;
    global_moderator.admin_role = AdminRole::Moderator;

    // normal user, simple moderator, global moderator cannot set any role
    assert_eq!(
        std_user.check_can_set_user_sphere_role(PermissionLevel::Moderate, test_user.person_id, sphere_name, &db_pool).await,
        Err(AppError::InsufficientPrivileges)
    );
    assert_eq!(
        simple_mod.check_can_set_user_sphere_role(PermissionLevel::Moderate, test_user.person_id, sphere_name, &db_pool).await,
        Err(AppError::InsufficientPrivileges)
    );
    assert_eq!(
        global_moderator.check_can_set_user_sphere_role(PermissionLevel::Moderate, test_user.person_id, sphere_name, &db_pool).await,
        Err(AppError::InsufficientPrivileges)
    );

    // manage mods can set user role for normal users, moderators with a lower level but not manage mods and leaders
    assert_eq!(
        manage_mod.check_can_set_user_sphere_role(PermissionLevel::Ban, test_user.person_id, sphere_name, &db_pool).await,
        Ok(())
    );
    assert_eq!(
        manage_mod.check_can_set_user_sphere_role(PermissionLevel::None, simple_mod.person_id, sphere_name, &db_pool).await,
        Ok(())
    );
    assert_eq!(
        manage_mod.check_can_set_user_sphere_role(PermissionLevel::Ban, simple_mod.person_id, sphere_name, &db_pool).await,
        Ok(())
    );
    assert_eq!(
        manage_mod.check_can_set_user_sphere_role(PermissionLevel::Manage, simple_mod.person_id, sphere_name, &db_pool).await,
        Err(AppError::InsufficientPrivileges)
    );
    assert_eq!(
        manage_mod.check_can_set_user_sphere_role(PermissionLevel::Lead, simple_mod.person_id, sphere_name, &db_pool).await,
        Err(AppError::InsufficientPrivileges)
    );
    assert_eq!(
        manage_mod.check_can_set_user_sphere_role(PermissionLevel::Moderate, manage_mod.person_id, sphere_name, &db_pool).await,
        Err(AppError::InsufficientPrivileges)
    );
    assert_eq!(
        manage_mod.check_can_set_user_sphere_role(PermissionLevel::Moderate, lead_user.person_id, sphere_name, &db_pool).await,
        Err(AppError::InsufficientPrivileges)
    );
    assert_eq!(
        manage_mod.check_can_set_user_sphere_role(PermissionLevel::Manage, lead_user.person_id, sphere_name, &db_pool).await,
        Err(AppError::InsufficientPrivileges)
    );
    ...
    assert_eq!(
        lead_user.check_can_set_user_sphere_role(PermissionLevel::Ban, test_user.person_id, sphere_name, &db_pool).await,
        Ok(())
    );
    assert_eq!(
        lead_user.check_can_set_user_sphere_role(PermissionLevel::Manage, test_user.person_id, sphere_name, &db_pool).await,
        Ok(())
    );
    assert_eq!(
        lead_user.check_can_set_user_sphere_role(PermissionLevel::Manage, simple_mod.person_id, sphere_name, &db_pool).await,
        Ok(())
    );
    assert_eq!(
        lead_user.check_can_set_user_sphere_role(PermissionLevel::None, manage_mod.person_id, sphere_name, &db_pool).await,
        Ok(())
    );
    assert_eq!(
        lead_user.check_can_set_user_sphere_role(PermissionLevel::Manage, lead_user.person_id, sphere_name, &db_pool).await,
        Err(AppError::InsufficientPrivileges)
    );
    assert_eq!(
        admin.check_can_set_user_sphere_role(PermissionLevel::Lead, test_user.person_id, sphere_name, &db_pool).await,
        Ok(())
    );
    assert_eq!(
        admin.check_can_set_user_sphere_role(PermissionLevel::Manage, simple_mod.person_id, sphere_name, &db_pool).await,
        Ok(())
    );
    assert_eq!(
        admin.check_can_set_user_sphere_role(PermissionLevel::Moderate, manage_mod.person_id, sphere_name, &db_pool).await,
        Ok(())
    );
    // An admin cannot reduce a leader's permission, but can instead set another user as leader
    assert_eq!(
        admin.check_can_set_user_sphere_role(PermissionLevel::None, lead_user.person_id, sphere_name, &db_pool).await,
        Err(AppError::InsufficientPrivileges)
    );
...
async fn test_create_user() -> Result<(), AppError> {
    let db_pool = get_db_pool().await;
    let user_1_value = "1";
    let db_user_1 = create_or_update_user(user_1_value, user_1_value, user_1_value, &db_pool).await.expect("Sql user 1 should be created");
    assert_eq!(db_user_1.oidc_id, user_1_value);
    assert_eq!(db_user_1.username, user_1_value);
    assert_eq!(db_user_1.email, user_1_value);
    assert_eq!(db_user_1.admin_role, AdminRole::None);
    assert_eq!(db_user_1.delete_timestamp, None);
    ...
    let db_user_2 = create_or_update_user(user_2_value, user_2_value, user_2_value, &db_pool).await.expect("Sql user 2 should be created");
    assert_eq!(db_user_2.oidc_id, user_2_value);
    assert_eq!(db_user_2.username, user_2_value);
    assert_eq!(db_user_2.email, user_2_value);
    assert_eq!(db_user_2.admin_role, AdminRole::None);
    assert_eq!(db_user_2.delete_timestamp, None);

    let user_1 = User::get(db_user_1.person_id, &db_pool).await.expect("Should be able to get user 1");
    assert_eq!(user_1.user_id, db_user_1.user_id);
    assert_eq!(user_1.oidc_id, db_user_1.oidc_id);
    assert_eq!(user_1.username, db_user_1.username);
    assert_eq!(user_1.email, db_user_1.email);
    assert_eq!(user_1.admin_role, db_user_1.admin_role);
    assert_eq!(user_1.delete_timestamp, db_user_1.delete_timestamp);

    let user_1_updated_value = "3";
    let db_user_1_updated = create_or_update_user(
        user_1_value,
        user_1_updated_value,
        user_1_updated_value,
        &db_pool
    ).await.expect("user 1 should be updated");
    assert_eq!(db_user_1_updated.oidc_id, user_1_value);
    assert_eq!(db_user_1_updated.username, user_1_updated_value);
    assert_eq!(db_user_1_updated.email, user_1_updated_value);
    assert_eq!(db_user_1_updated.admin_role, AdminRole::None);
    assert_eq!(db_user_1_updated.delete_timestamp, None);
